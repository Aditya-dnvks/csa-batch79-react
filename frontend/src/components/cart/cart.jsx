...
const Cart = () => {
  ...
  const handleAddQuantity = (id) => {
    const newCart = cartData.map((each) =>
      each._id === id ? { ...each, quantity: each.quantity + 1 } : each
    );
  ...
  const handleDelete = (id) => {
    const newCart = cartData.filter((each) => each._id !== id); // rest all items --> Cart Item --> Currentitem
    localStorage.setItem("cartData", JSON.stringify(newCart));
    setCartData(newCart);
  };
    ...
    <div>
      {cartData.map((each) => (
        <div className="flex border justify-between border-black rounded-xl p-3 w-[60vw] m-3">
          <div
            className="flex"
            onClick={() => navigation(`/item-details/${each._id}`)}
          >
            <img
              src={each.image}
              width={150}
          ...
          <div className="my-auto text-center">
            <div className="flex h-fit items-center my-auto gap-3 justify-center border-1 border-orange-500 w-fit rounded-md">
              <Button onClick={() => handleAddQuantity(each._id)}>+</Button>
              <h1>{each.quantity}</h1>
              <Button onClick={() => handleRemoveQuantity(each._id)}>-</Button>
            </div>
            <DeleteIcon onClick={() => handleDelete(each._id)} />
          </div>
        </div>
      ))}
    </div>
